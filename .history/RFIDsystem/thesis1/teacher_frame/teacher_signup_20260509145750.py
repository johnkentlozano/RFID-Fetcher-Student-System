...
class TeacherSignUpFrame(tk.Frame):
    ...
    def left_image(self):
        ...
        if img_path:
            ...
            tk.Label(
                left,
                text=f"COULD NOT FIND: {target_file}",
                fg="red",
                bg="#E0E0E0",
                font=("Arial", 9)
            ).pack(expand=True)
    ...
    def signup_panel(self):
        ...
        # -------- RFID --------
        self.rfid_uid = self.entry(
            form_container,
            "Tap RFID (Optional)",
            validate_type="alphanumeric"
        )
        ...
        back = tk.Button(
            ...
            font=("Arial", 9),
            cursor="hand2",
            command=lambda: self.controller.show_frame("TeacherLoginFrame")
        )
        back.pack()
    ...
    self,
    text="Back to Login",
    command=lambda: controller.show_frame("TeacherLoginFrame"),
    bg="gray",
    fg="white"
).pack(pady=10)
    ...
    def entry(self, panel, text, hide=False, validate_type=None):
        ...
        if validate_type == "numeric":
            vcmd = (self.register(self.validate_employee_id), '%P')
            e.config(validate="key", validatecommand=vcmd)
    ...
    def validate_password(self, event=None):
        ...
        self.pw_reqs["length"].config(
            fg="green" if len(pw) >= 8 else "red"
        )

        self.pw_reqs["upper"].config(
            fg="green" if any(c.isupper() for c in pw) else "red"
        )

        self.pw_reqs["digit"].config(
            fg="green" if any(c.isdigit() for c in pw) else "red"
        )
    ...
    def signup(self):
        ...
        try:
            with db_connect() as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        """
                        INSERT INTO users
                        (username, password, employee_id, role, rfid_uid)
                        VALUES (%s, %s, %s, %s, %s)
                        """,
                        (
                            user,
            ...
            self.controller.show_frame("TeacherLoginFrame")
