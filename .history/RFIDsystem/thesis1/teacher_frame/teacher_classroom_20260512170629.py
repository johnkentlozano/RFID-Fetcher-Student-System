...
class ClassroomFrame(tk.Frame):
        ...
        try:
            ...
            with db_connect() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT Teacher_name FROM teacher WHERE employee_id = %s",
                        (self.username,)
                    )
                    res = cur.fetchone()
    ...
    def remove_student_from_class(self):
        ...
        try:
            with db_connect() as conn:
                with conn.cursor() as cur:
                    cur.execute("DELETE FROM classroom WHERE student_id = %s AND employee_id = %s", (sid, self.real_teacher_name))
                    conn.commit()
            self.refresh_tables()
        ...
        try:
            with db_connect() as conn:
                ...
                # Use time_out instead of id to find the latest log if 'id' is missing
                    cur.execute("SELECT student_name, time_out FROM history_log WHERE employee_id = %s ORDER BY time_out DESC LIMIT 1", (self.real_teacher_name,))
