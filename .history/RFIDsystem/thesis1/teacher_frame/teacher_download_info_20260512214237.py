...
class TeacherDownloadFrame(tk.Frame):
        ...
        try:
            ...
            with db_connect() as conn:
                with conn.cursor() as cur:
                    # 🔥 IMPORTANT: sync with classroom table (NOT history_log only)
                    cur.execute("""
        ...
        try:
            ...
            with db_connect() as conn:
                with conn.cursor() as cur:
                    ...
                    # 🔥 KEY FIX: JOIN classroom so ONLY enrolled students appear
                    query = """
                        SELECT 
                            h.student_name,
                            h.student_id,
                            ...
                            h.location
                        FROM history_log h
                        JOIN classroom c ON h.student_id = c.student_id
                        WHERE h.employee_id = %s
                        AND c.employee_id = %s
                        AND DATE(h.time_out) BETWEEN %s AND %s
                    """

                    params = [employee_id, employee_id, start, end]
    ...
    def export_excel(self):
        ...
        wb.save(file_path)
        messagebox.showinfo("Success", "Export done!")
