...
class ForgotPasswordFrame(tk.Frame):
    ...
    def forgot_panel(self):
        ...
        # -------- Username --------
        tk.Label(container, text="Username", bg="white",
        ...
        # -------- Employee ID --------
        tk.Label(container, text="Employee ID", bg="white",
                 font=("Arial", 9, "bold")).pack(anchor="w")
        ...
        # -------- New Password --------
        tk.Label(container, text="New Password", bg="white",
                 font=("Arial", 9, "bold")).pack(anchor="w")
        self.new_pw = tk.Entry(
            container,
            font=("Arial", 12),
            bg="#F8F9FA",
            bd=0,
        ...
        try:
            with db_connect() as conn:
                with conn.cursor() as cur:
                    ...
                    # ✅ CHECK TEACHER ONLY
                    cur.execute(
                        "SELECT role FROM users WHERE username=%s AND employee_id=%s AND role='Teacher'",
                        (user, emp_id)
                    )
                    result = cur.fetchone()
                    ...
                    if result:
                        # ✅ UPDATE ONLY TEACHER
                        cur.execute(
                            "UPDATE users SET password=%s WHERE username=%s AND employee_id=%s AND role='Teacher'",
                            (hashed, user, emp_id)
                        )
                        conn.commit()
                        ...
                        messagebox.showerror(
                            "Error",
                            "Teacher account not found or details incorrect."
                        )
