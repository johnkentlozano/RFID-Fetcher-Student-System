...
class TeacherLoginFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(self, text="Teacher Login", font=("Arial", 20)).pack(pady=20)
        ...
        tk.Button(
            self,
            text="Sign Up",
            command=lambda: controller.show_frame("TeacherSignUpFrame")
        ).pack(pady=5)
    ...
    def login(self):
        ...
        # 🔥 TEMP LOGIN (replace with DB)
        if user == "teacher" and pw == "1234":
            user_data = {"username": user, "role": "Teacher"}
    ...
    def login_panel(self):
        ...
        tk.Label(
            input_container,
            text="Tap Teacher RFID",
            bg="white",
            font=("Arial", 10, "bold")
        ).pack(anchor="w")

        self.rfid_uid = tk.Entry(
            input_container,
            font=("Arial", 12),
            bg="#F8F9FA",
            bd=0,
    ...
    def login(self):
        ...
        if result:
            ...
            if emp_id != str(stored_emp_id):
                messagebox.showerror("Error", "Teacher ID is incorrect")
                return
    ...
    def rfid_login(self, event=None):
            ...
            with db_connect() as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute("""
                        ...
                        FROM users
                        WHERE rfid_uid=%s AND role='Teacher'
                    """, (rfid,))
        ...
        if result:
            ...
            messagebox.showinfo("Success", "Teacher RFID Login Successful")
            self.controller.login_success(user_data)

        else:
            messagebox.showerror("Error", "Teacher RFID not registered")
