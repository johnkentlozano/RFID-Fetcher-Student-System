...
class ClassroomFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.current_photo = None  # Prevent image garbage collection
        ...
        try:
            with db_connect() as conn:
                with conn.cursor() as cur:
                    # Adjust column name if your table uses 'name' instead of 'employee_id' for display
    ...
    def setup_tables(self, parent):
        ...
        self.reset_btn = tk.Button(btn_frame, text="RESET CLASS (NEW YEAR)", bg="#607D8B", fg="white", font=("Arial", 9, "bold"), width=16, command=self.clear_entire_class)
    ...
    def load_full_student_details(self, student_id):
        try:
            with db_connect() as conn:
                # Standard cursor for better compatibility
                with conn.cursor() as cur:
