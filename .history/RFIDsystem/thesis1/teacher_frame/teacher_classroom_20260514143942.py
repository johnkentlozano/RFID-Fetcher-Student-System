...
class ClassroomFrame(tk.Frame):
    ...
    def get_teacher_display_name(self):
        # FIX: guard against None employee_id before querying
    ...
    def load_full_student_details(self, student_id):
        # FIX: guard against None student_id
        if not student_id:
            return
        try:
            with db_connect() as conn:
    ...
    def load_user(self, user_data):
        # FIX: guard against None or invalid user_data before any .get() calls
        ...
        # FIX: only fetch display name if employee_id is actually set
