...
class TeacherDownloadFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # LEFT PANEL (DETAILS)
        self.detail_frame = tk.Frame(main_frame, bg="white", bd=1, relief="solid", width=250)
        self.detail_frame.pack(side="left", fill="y", padx=(0, 10))

        tk.Label(self.detail_frame, text="📋 Student Details",
                 font=("Segoe UI", 12, "bold"), bg="white").pack(pady=10)

        self.detail_labels = {}
        fields = ["Name", "ID", "Time Out", "Fetcher", "Location"]

        for field in fields:
            lbl = tk.Label(self.detail_frame, text=f"{field}: ",
                           anchor="w", bg="white", font=("Segoe UI", 10))
    ...
    def show_details(self, event):
        ...
        for i, field in enumerate(fields):
            self.detail_labels[field].config(text=f"{field}: {values[i]}")
