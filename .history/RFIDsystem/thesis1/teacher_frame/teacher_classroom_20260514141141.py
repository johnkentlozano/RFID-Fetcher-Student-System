...
class ClassroomFrame(tk.Frame):
    ...
    def get_teacher_display_name(self):
        try:
            ...
            return self.username if hasattr(self, "username") else "Unknown Teacher"
    ...
    def verify_student_id(self, *args):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Verify Error: {e}")
    ...
    def add_student_to_class(self):
        ...
        try:
            with db_connect() as conn:
                ...
                # CHECK IF already assigned to SAME teacher
                    cur.execute("""
    ...
    def remove_student_from_class(self):
        ...
        if not messagebox.askyesno("Remove", f"Remove {sname} from your class?"):
            return
        ...
        try:
            with db_connect() as conn:
                with conn.cursor() as cur:
                    ...
                    for row in rows:
                        clean_row = tuple("N/A" if r is None else r for r in row)
        ...
        try:
            with db_connect() as conn:
                with conn.cursor(dictionary=True) as cur:
                    ...
                    if not student:
                        self.info_label.config(text="No student data found")
                        return

                    name = student.get("Student_name") or "N/A"
                    grade = student.get("grade_lvl") or "N/A"
                    photo_blob = student.get("photo_path")
                ...
                # ✅ SAFE IMAGE LOAD
                    if photo_blob:
                        try:
                            ...
                            self.photo_label.config(image='', text="Invalid Image")
                    else:
    ...
    # SAFE ACCESS (works for dict OR tuple)
                        if isinstance(log, dict):
                            time_out = log.get("time_out", "N/A")
                            fetcher = log.get("fetcher_name", "N/A")
                            location = log.get("location", "N/A")
                        else:
        # fallback if tuple
                            time_out = log[0] if len(log) > 0 else "N/A"
                            fetcher = log[1] if len(log) > 1 else "N/A"
        location = log[2] if len(log) > 2 else "N/A"
            ...
            with db_connect() as conn:
                ...
                # 1. Check if it's the normal Fetcher/Parent
                    ...
                    cur.execute("""
                        ...
                        JOIN teacher_rfid_registration tr ON c.employee_id = (
    ...
    def load_user(self, user_data):
        ...
        self.after(3000, self.auto_refresh_class)
