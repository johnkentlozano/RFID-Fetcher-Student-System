...
class AdminOverrideFrame(tk.Frame):
    ...
    def set_ui_state(self, state):
        if state == "idle":
            ...
            self.cancel_btn.pac1111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111k_forget()
    ...
    def refresh_list(self):
        ...
        try:
            ...
            messagebox.showerror(f"List Refresh Error: {e}")

        #handle delete
    def handle_delete(self):
        selected = self.tree.focus()
        if not selected:
            messagebox.showwarning ("please selected to delete")
            return
        eid = self.tree.item(selected, "values")[0]
        messagebox.askyesno("Are you sure you want to delete?")
        try:
            with db_connect() as conn:
                with conn.cursor() as cur:
                    cur.execute ("Delete employee_id admin_rfid_registration WHERE employee_id=%s",(eid))
                    conn.commit()
            self.refresh_list()
        except Exception as e:
